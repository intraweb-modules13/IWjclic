...
switch($beans[0]['ID']){
	case "get_properties":
		...
		$settings = ModUtil::apiFunc('IWjclic', 'user', 'getSettings');
	...
	case "add session":
		...
		$createdSession = ModUtil::apiFunc('IWjclic', 'user', 'createSession', array('items' => $items));
		//create a user session
		if($createdSession && !ModUtil::apiFunc('IWjclic', 'user', 'getUserSession', array('user_id' => $beans[0]['PARAMS']['user'],
													'key' => $key))){

			ModUtil::apiFunc('IWjclic', 'user', 'createUser', array('user_id' => $beans[0]['PARAMS']['user'],
										'key' => $key));
		}		
	...
	case "multiple":
		foreach ($beans as $bean){
			if ($bean['ID']=='add activity'){
				...
				$createdSession = ModUtil::apiFunc('IWjclic', 'user', 'addActivity', array('items' => $items));
			}			
		}
		echo '<?xml version="1.0" encoding="UTF-8"?'.'>';
